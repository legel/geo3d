...
2. **Format conversion:**
    ...
    The transforms file will need to be saved as `transforms.json` in the output directory. The images will be saved in a subfolder called `images`. Note that the default OPF output is `transforms_train.json` and the images are not moved.
