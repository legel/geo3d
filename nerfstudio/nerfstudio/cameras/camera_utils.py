...
def get_interpolated_poses_many(
    ...
    # nerfstudio/nerfstudio/cameras/camera_utils.py:308: UserWarning: To copy construct from a tensor, it is recommended to use sourceTensor.detach().clone() or sourceTensor.detach().clone().requires_grad_(True), rather than torch.tensor(sourceTensor).
    #   torch.tensor(k_interp, dtype=torch.float32),
    #
    return (
        torch.tensor(traj, dtype=torch.float32),  # Different since currently numpy
        k_interp.detach().clone().to(torch.float32),
        time_interp.detach().clone().to(torch.float32) if time_interp is not None else None,
    )
